...
**AIエージェント・オーケストレーションシステム — Multi-CLI対応**

*コマンド1つで、10体のAIエージェントが並列稼働 — **Claude Code / OpenAI Codex / GitHub Copilot / Kimi Code** マルチCLI対応*
...
<p align="center">
  <img src="images/screenshots/hero/latest-translucent-20260210-190453.png" alt="オーケストレーターペインでの最新半透過セッションキャプチャ" width="940">
</p>

<p align="center">
  <img src="images/screenshots/hero/latest-translucent-20260208-084602.png" alt="オーケストレーターペインでの自然言語コマンド入力" width="420">
  <img src="images/company-creed-all-panes.png" alt="コーディネーターとエグゼキューターが全ペインで並列反応する様子" width="520">
</p>

<p align="center"><i>コーディネーター1体がエグゼキューター7体+アナライザー1体を統率 — 実際の稼働画面、モックデータなし</i></p>

---

## これは何？

**multi-agent-shogun** は、複数のAIコーディングCLIインスタンスを同時に実行し、階層型エージェントアーキテクチャで統率するシステムです。**Claude Code**、**OpenAI Codex**、**GitHub Copilot**、**Kimi Code** の4CLIに対応。

**なぜ使うのか？**
- 1つの命令で、7体のAIワーカー+1体のアナライザーが並列で実行
...
```
    ...
    └──────┬──────┘
           │
  ┌─┬─┬─┬─┴─┬─┬─┬────────┐
  │1│2│3│4│5│6│7│ GUNSHI │  ← 7体のワーカー + 1体のアナライザー
  └─┴─┴─┴─┴─┴─┴─┴────────┘
     ASHIGARU    アナライザー
```
...
**実績のある階層構造** — オーケストレーター→コーディネーター→エグゼキューターの指揮系統が設計レベルで衝突を防止：明確な責任分担、エージェントごとの専用ファイル、イベント駆動通信、ポーリングなし。
...
**「AIを使い倒す」思想** — 定額CLIサブスクなら、8体のエグゼキューターを気兼ねなく投入できる。1時間稼働でも24時間稼働でもコストは同じ。「まあまあ」と「徹底的に」の二択で悩む必要がない — エージェントを増やせばいい。

### Multi-CLI対応

このシステムは特定ベンダーに依存しない。4つのCLIツールに対応し、それぞれの強みを活かす：
...
└── roles/               # ロール定義（オーケストレーター、コーディネーター、エグゼキューター）
    ↓ ビルド
...
エグゼキューターがタスクを実行する中で、**再利用可能なパターンを自動的に発見**し、スキル候補として提案します。コーディネーターが提案を `dashboard.md` に集約し、ユーザー（あなた）が正式なスキルに昇格させるか判断します。
...
YAMLで報告:  skill_candidate:
    ...
    ↓
dashboard.md に掲載 → ユーザーが承認 → .claude/commands/ にスキル作成
    ↓
...
ベッドから、カフェから、トイレから。スマホでAIエージェントを操作できる。
...
3. スマホのTermuxから：
   ...
   css    # オーケストレーターに繋がる
   ```
4. ＋ボタンで新しいウィンドウを開いて、エージェントの様子も見る：
   ```sh
   ssh あなたのユーザー名@あなたのTailscale IP
   csm    # コーディネーター+エグゼキューターの9ペインが広がる
   ```

**切り方：** Termuxのウィンドウをスワイプで閉じるだけ。tmuxセッションは生き残る。AIエージェントは黙々と作業を続けている。

**音声入力：** スマホの音声入力で喋れば、オーケストレーターが自然言語を理解して全エージェントに指示を出す。音声認識の誤字も文脈で解釈してくれる。
...
| `launch.sh` | tmuxセッション作成 + Claude Code起動 + 指示書読み込み + ntfyリスナー起動 | 毎日 |
...
| エージェント | 役割 | 数 |
|-------------|------|-----|
| 🎛️ オーケストレーター（Shogun） | 司令塔 - あなたの命令を受ける | 1 |
| 📋 コーディネーター（Karo） | 管理者 - タスク分配・簡易QC・ダッシュボード管理 | 1 |
| 🚀 エグゼキューター（Ashigaru） | ワーカー - 実装タスクを並列実行 | 7 |
| 🧠 アナライザー（Gunshi） | 分析役 - 分析・評価・設計など高度な思考タスク | 1 |

tmuxセッションが作成されます：
- `shogun` - ここに接続してコマンドを出す
- `multiagent` - コーディネーター・エグゼキューター・アナライザーがバックグラウンドで稼働
...
`launch.sh` 実行後、全エージェントが自動的に指示書を読み込み、作業準備完了となります。

新しいターミナルを開いてオーケストレーターに接続：
...
その間、コーディネーターはタスクをエグゼキューターに分配し、並列実行します。
...
| ワーカー | タスク | 状態 |
|----------|--------|------|
| エグゼキューター 1 | React調査 | 実行中 |
| エグゼキューター 2 | Vue調査 | 実行中 |
| エグゼキューター 3 | Angular調査 | 完了 |
```
...
オーケストレーターがタスクを `queue/shogun_to_karo.yaml` に書き込み、コーディネーターを起動。あなたには即座に制御が戻ります。

コーディネーターがタスクをサブタスクに分解：

| ワーカー | 割当内容 |
|----------|----------|
| エグゼキューター 1 | Notion MCP調査 |
| エグゼキューター 2 | GitHub MCP調査 |
| エグゼキューター 3 | Playwright MCP調査 |
| エグゼキューター 4 | Memory MCP調査 |
| エグゼキューター 5 | Sequential Thinking MCP調査 |

5体のエグゼキューターが同時に調査開始。リアルタイムで作業を見ることができます。

<p align="center">
  <img src="images/company-creed-all-panes.png" alt="エグゼキューターがtmux全ペインで並列実行する様子" width="900">
</p>
...
オーケストレーターは即座に委譲して、あなたに制御を返します：

```
あなた: 命令 → オーケストレーター: 委譲 → あなた: 次の命令をすぐ出せる
                                    ↓
                    ワーカー: バックグラウンドで実行
                                    ↓
                    ダッシュボード: 結果を表示
...
3. Memory MCP 読み込み → ユーザーの好みを復元（~700トークン）
...
スマホとオーケストレーターの間で双方向通信 — SSH不要、Tailscale不要、サーバ不要。

| 方向 | 仕組み |
|------|--------|
| **スマホ → オーケストレーター** | ntfyアプリからメッセージを送信 → `ntfy_listener.sh` がストリーミングで受信 → オーケストレーターが自動処理 |
| **コーディネーター → スマホ（直接）** | コーディネーターが `dashboard.md` を更新する際、`scripts/ntfy.sh` 経由で直接プッシュ通知を送信 — **オーケストレーターを経由しない**（オーケストレーターは人間との対話用、進捗報告用ではない） |

```
📱 あなた（ベッドから）       🎛️ オーケストレーター
    │                          │
    │  "React 19を調査せよ"    │
    ├─────────────────────────►│
    │    (ntfyメッセージ)      │  → コーディネーターに委譲 → エグゼキューターが作業
    │                          │
    │  "✅ cmd_042 完了"       │
    │◄─────────────────────────┤
    │    (プッシュ通知)        │
...
> **⚠️ セキュリティ注意:** トピック名がそのままパスワードです。知っている人は誰でも通知を読んだり、オーケストレーターにメッセージを送れてしまいます。推測されにくい名前を選び、**スクリーンショットやブログ、GitHubコミットなどで公開しないでください**。

**動作確認:**

```bash
# テスト通知をスマホに送信
bash scripts/ntfy.sh "エージェントシステムからのテスト通知 🎛️"
```
...
**スマホからオーケストレーターに指示を送る方法:**

1. スマホでntfyアプリを開く
2. 購読しているトピックをタップ
3. メッセージを入力（例: `React 19のベストプラクティスを調査して`）して送信
4. `ntfy_listener.sh` が受信 → `queue/ntfy_inbox.yaml` に書き込み → オーケストレーターを起こす
5. オーケストレーターがメッセージを読み、通常のコーディネーター→エグゼキューターパイプラインで処理

送信したテキストがそのままコマンドになります。オーケストレーターに話しかけるように書けばOK — 特別な構文は不要です。

**リスナーの手動起動**（`launch.sh` を使わない場合）:
...
リスナーは接続が切れても自動的に再接続します。`launch.sh` で起動すれば自動起動されるため、手動起動は起動スクリプトを使わない場合のみ必要です。
...
| スマホ→オーケストレーターが動かない | リスナーが稼働中か確認: `pgrep -f ntfy_listener.sh` |
| メッセージがオーケストレーターに届かない | `queue/ntfy_inbox.yaml` を確認 — メッセージがあればオーケストレーターが処理中の可能性 |
| "ntfy_topic not configured" エラー | `config/settings.yaml` に `ntfy_topic: "your-topic"` を追加 |
| 通知が重複する | 再接続時の正常動作 — オーケストレーターがメッセージIDで重複排除します |
...
- コーディネーターがタスク割当・完了時に自動更新
- 9ペインを一目見れば、誰が何をしているか即座にわかる

### 🔊 10. シャウトモード（完了エコー）

エグゼキューターがタスクを完了すると、パーソナライズされたClaude Code風のメッセージをtmuxペインに表示します — エージェントが働いている実感を得られる。

```
┌ ashigaru1 (Sonnet) ──────────┬ ashigaru2 (Sonnet) ──────────┐
│                               │                               │
│  🚀 エグゼキューター1、       │  🚀 エグゼキューター2、       │
│  タスク完了！Ship it!         │  タスク完了！Ship it!         │
│  ❯                            │  ❯                            │
└───────────────────────────────┴───────────────────────────────┘
```
...
コーディネーターがタスクYAMLに `echo_message` フィールドを記述。エグゼキューターは全作業完了後（レポート + inbox通知の後）、**最後のアクション**として `echo` を実行。メッセージは `❯` プロンプト直上に残る。
...
task:
  task_id: subtask_001
  description: "比較表を作成"
  echo_message: "🚀 エグゼキューター1、タスク完了！Ship it!"
```

**シャウトモードがデフォルト。** 無効にする場合（echoのAPIトークン節約）:

```bash
./launch.sh --silent    # 完了エコーなし
./launch.sh             # デフォルト: シャウトモード（完了エコー有効）
```

サイレントモードは `DISPLAY_MODE=silent` をtmux環境変数に設定。コーディネーターがタスクYAML作成時にこれを確認し、`echo_message` フィールドを省略する。
...
```
 🗣️ 「牛乳買う、歯医者 明日、請求書 金曜まで」
       ...
       ▼
 ┌──────────────────────────────┐
 │  ntfy → オーケストレーター  │  AIが自動分類、日付解析、優先度設定
 └────────┬─────────────────────┘
          │
          ▼
 ┌──────────────────┐
...
**Q: multi-agent-shogunシステム全体なしでSayTaskだけ使える？**
A: SayTaskはこのシステムの機能の一部。multi-agent-shogunはスタンドアロンのマルチエージェント開発プラットフォームとしても機能する — 1つのシステムで両方の機能が手に入る。
...
| エージェント | モデル | 思考モード | 役割 |
|-------------|--------|----------|------|
| オーケストレーター | Opus | **有効（high）** | ユーザーのアドバイザー。`--shogun-no-thinking` で中継専用モードに |
| コーディネーター | Sonnet | 有効 | タスク分配・簡易QC・ダッシュボード管理 |
| アナライザー | Opus | 有効 | 深い分析・設計レビュー・アーキテクチャ評価 |
| エグゼキューター1-7 | Sonnet | 有効 | 実装：コード・リサーチ・ファイル操作 |

**認知的複雑さ**でタスクを分割するのがこのシステムの設計思想。エグゼキューターが実装（L1-L3）を担当し、アナライザーが深い推論（L4-L6）を担当する。モデルの動的切り替えは不要 — 最初から適切なエージェントに適切なタスクが届く。
...
| レベル | カテゴリ | 内容 | ルーティング先 |
|--------|----------|------|---------------|
| L1 | 記憶 | 事実の想起、コピー、一覧化 | **エグゼキューター** |
| L2 | 理解 | 説明、要約、言い換え | **エグゼキューター** |
| L3 | 応用 | 手順の実行、既知パターンの実装 | **エグゼキューター** |
| L4 | 分析 | 比較、調査、構造の分解 | **アナライザー** |
| L5 | 評価 | 判断、批評、推奨 | **アナライザー** |
| L6 | 創造 | 設計、構築、新しいソリューションの統合 | **アナライザー** |

コーディネーターが各サブタスクにBloomレベルを付与し、適切なエージェントにルーティング。L1-L3はエグゼキューターに並列分配、L4-L6はアナライザーへ。簡単なL4タスク（小規模なコードレビュー等）は、コーディネーターの判断でエグゼキューターに回すこともある。
...
task:
  ...
  blockedBy: ["subtask_010a"]  # エグゼキューター1のタスク完了を待つ
  description: "subtask_010aで構築したAPIクライアントを統合"
```

ブロック元のタスクが完了すると、コーディネーターが自動的に依存タスクのブロックを解除し、空いているエグゼキューターに割り当てます。これにより待機時間が削減され、依存タスクの効率的なパイプライン処理が可能になります。

### 動的モデルルーティング（capability_tiers）

エージェント単位のルーティングに加え、**エグゼキューター階層内でのモデルレベルルーティング**も設定できます。`config/settings.yaml` に `capability_tiers` テーブルを定義し、各モデルのBloom上限を指定します：
...
このシステムは5つの核心原則に基づいて設計されている：

| 原則 | 説明 |
|------|------|
| **自律構成設計** | テンプレートではなく、タスクの複雑さに応じてエージェント構成を設計 |
...
### なぜ階層構造（オーケストレーター→コーディネーター→エグゼキューター）なのか

1. **即座の応答**: オーケストレーターは即座に委譲し、あなたに制御を返す
2. **並列実行**: コーディネーターが複数のエグゼキューターに同時分配
3. **単一責任**: 各役割が明確に分離され、混乱しない
4. **スケーラビリティ**: エグゼキューターを増やしても構造が崩れない
5. **障害分離**: 1体のエグゼキューターが失敗しても他に影響しない
6. **人間への報告一元化**: オーケストレーターだけが人間とやり取りするため、情報が整理される
...
各ペインに `@agent_id` というtmuxユーザーオプションを設定（例: `karo`, `ashigaru1`）。`pane_index` はペイン再配置でズレるが、`@agent_id` は `launch.sh` が起動時に固定設定するため変わらない。
...
### なぜ dashboard.md はコーディネーターのみが更新するのか

1. **単一更新者**: 競合を防ぐため、更新責任者を1人に限定
2. **情報集約**: コーディネーターは全エグゼキューターの報告を受ける立場なので全体像を把握
3. **一貫性**: すべての更新が1つの品質ゲートを通過
4. **割り込み防止**: オーケストレーターが更新すると、ユーザーの入力中に割り込む恐れあり
...
スキルは `/スキル名` で呼び出し可能。オーケストレーターに「/スキル名 を実行」と伝えるだけ。
...
```
エグゼキューターが作業中にパターンを発見
    ↓
dashboard.md の「スキル化候補」に上がる
    ↓
ユーザー（あなた）が内容を確認
    ↓
承認すればコーディネーターに指示してスキルを作成
```
...
実行される処理:
1. オーケストレーターがコーディネーターに委譲
2. コーディネーターが割り当て:
   - エグゼキューター1: GitHub Copilotを調査
   - エグゼキューター2: Cursorを調査
   - エグゼキューター3: Claude Codeを調査
   - エグゼキューター4: Codeiumを調査
   - エグゼキューター5: Amazon CodeWhispererを調査
3. 5体が同時に調査
...
実行される処理:
1. コーディネーターがMCP経由でNotionコンテンツを取得
2. エグゼキューター2: 確認すべき項目をリスト化
3. エグゼキューター3: 技術的な実現可能性を調査
4. エグゼキューター4: PoC計画書を作成
...
オーケストレーターに「最新のスクショを見ろ」と伝えるだけで、スクリーンキャプチャを読み取って分析します。（Windowsでは `Win+Shift+S`）
...
スマホの [ntfyアプリ](https://ntfy.sh) で同じトピックをサブスクライブしてください。リスナーは `launch.sh` で自動起動します。
...
# フルパワーモード: 全エグゼキューターをOpusで起動（最大能力・高コスト）
./launch.sh -k
./launch.sh --kessen

# サイレントモード: 完了エコーを無効化（echoのAPIトークン節約）
./launch.sh -S
./launch.sh --silent
...
# オーケストレーター中継専用モード: オーケストレーターのThinkingを無効化（コスト節約）
./launch.sh --shogun-no-thinking
...
```bash
alias csst='cd /mnt/c/tools/multi-agent-shogun && ./launch.sh'
alias css='tmux attach-session -t shogun'      # オーケストレーターウィンドウの起動
alias csm='tmux attach-session -t multiagent'  # コーディネーター・エグゼキューターウィンドウの起動
```
...
│   ├── shogun.md             # オーケストレーターの指示書
│   ├── karo.md               # コーディネーターの指示書
│   ├── ashigaru.md           # エグゼキューターの指示書
│   ├── gunshi.md             # アナライザーの指示書
│   └── cli_specific/         # CLI固有のツール説明
│       ├── claude_tools.md   # Claude Code ツール・機能
│       └── copilot_tools.md  # GitHub Copilot CLI ツール・機能
│
...
│
├── queue/                    # 通信ファイル
│   ├── shogun_to_karo.yaml   # オーケストレーターからコーディネーターへのコマンド
│   ├── ntfy_inbox.yaml       # スマホからの受信メッセージ（ntfy）
│   ├── inbox/                # エージェント別inboxファイル
│   │   ├── shogun.yaml       # オーケストレーターへのメッセージ
│   │   ├── karo.yaml         # コーディネーターへのメッセージ
│   │   └── ashigaru{1-8}.yaml # 各エグゼキューターへのメッセージ
│   ├── tasks/                # 各ワーカーのタスクファイル
│   └── reports/              # ワーカーレポート
│
...
この分離設計により、このシステムは複数の外部プロジェクトを横断的に統率しつつ、プロジェクトの詳細情報はバージョン管理の対象外に保つことができる。
...
<details>
<summary><b>オーケストレーターやエージェントが落ちた？（Claude Codeプロセスがkillされた）</b></summary>
...
# 方法2: コーディネーターがrespawn-paneで強制再起動（ネストも解消される）
...
| コマンド | 説明 |
|----------|------|
| `tmux attach -t shogun` | オーケストレーターに接続 |
| `tmux attach -t multiagent` | ワーカーに接続 |
| `Ctrl+B` の後 `0-8` | ペイン間を切り替え |
| `Ctrl+B` の後 `d` | デタッチ（実行継続） |
| `tmux kill-session -t shogun` | オーケストレーターセッションを停止 |
...
- **Bloom→エージェントルーティング** — 動的モデル切り替えをエージェントレベルのルーティングに置換。L1-L3タスク→エグゼキューター（Sonnet）、L4-L6タスク→アナライザー（Opus）。セッション中の `/model opus` 昇格は不要に — コーディネーターが最初から適切なエージェントにルーティング
- **アナライザー（Gunshi）がファーストクラスエージェントに** — ペイン8の分析役。深い分析、設計レビュー、アーキテクチャ評価、複雑なQCを担当。エグゼキューターは実装に専念
...
- **モデルデフォルト更新** — コーディネーター: Opus→Sonnet。全エグゼキューター: Sonnet（統一）。アナライザー: Opus（深い推論）
...
- **独立レート制限** — SparkはGPT-5.3-Codexとは独立したレート制限枠で動作。異なるエグゼキューターに両モデルを割り当てれば**実効スループットが2倍**に
- **起動時表示** — `launch.sh` が汎用的なエフォートレベルの代わりに実際のモデル名（例: `codex/gpt-5.3-codex-spark`）を表示
...
- **シャウトモード**（デフォルト）— エグゼキューターがタスク完了時にパーソナライズされたClaude Code風のメッセージを表示。`--silent` で無効化
...
- **フルパワーモード**（`-k` フラグ）— 全エグゼキューターOpusの最大能力構成
