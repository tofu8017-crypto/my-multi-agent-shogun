#!/bin/bash
# 🚀 multi-agent-shogun 起動スクリプト（毎日の起動用）
# Daily Deployment Script for Multi-Agent Orchestration System
#
# 使用方法:
#   ./launch.sh           # 全エージェント起動（前回の状態を維持）
#   ./launch.sh -c        # キューをリセットして起動（クリーンスタート）
...
# エグゼキューターIDリストと人数を動的に取得（settings.yaml から）
if [ "$CLI_ADAPTER_LOADED" = true ]; then
    _ASHIGARU_IDS_STR=$(get_ashigaru_ids)
else
    _ASHIGARU_IDS_STR="ashigaru1 ashigaru2 ashigaru3 ashigaru4"
fi
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            echo ""
            echo "🚀 multi-agent-shogun 起動スクリプト"
            echo ""
            ...
            echo "  -k, --kessen        フルパワーモード（全エグゼキューターをOpusで起動）"
            echo "                      未指定時は通常モード（エグゼキューター1-7=Sonnet, コーディネーター=Opus）"
            echo "  -s, --setup-only    tmuxセッションのセットアップのみ（Claude起動なし）"
            echo "  -t, --terminal      Windows Terminal で新しいタブを開く"
            echo "  -shell, --shell SH  シェルを指定（bash または zsh）"
            echo "                      未指定時は config/settings.yaml の設定を使用"
            echo "  -S, --silent        サイレントモード（エグゼキューターのタスク完了echo表示を無効化・API節約）"
            echo "                      未指定時はshoutモード（タスク完了時にecho表示）"
            echo "  -h, --help          このヘルプを表示"
            echo ""
            echo "例:"
            echo "  ./launch.sh              # 前回の状態を維持して起動"
            ...
            echo "  ./launch.sh -k           # フルパワーモード（全エグゼキューターOpus）"
            echo "  ./launch.sh -c -k         # クリーンスタート＋フルパワーモード"
            echo "  ./launch.sh -shell zsh   # zsh用プロンプトで起動"
            echo "  ./launch.sh --shogun-no-thinking  # オーケストレーターのthinkingを無効化（中継特化）"
            echo "  ./launch.sh -S           # サイレントモード（echo表示なし）"
            echo ""
            echo "モデル構成（少数精鋭4体制）:"
            echo "  オーケストレーター:  Opus 4.6（戦略・意思決定）"
            echo "  コーディネーター:    Opus 4.6（タスク分解・管理）"
            echo "  エグゼキューター1:   Haiku 4.5（偵察・軽作業）"
            echo "  エグゼキューター2-4: Sonnet 4.6（実装・テスト）"
            echo ""
            echo "チーム構成:"
            echo "  通常モード（デフォルト）:     エグゼキューター1=Haiku, エグゼキューター2-4=Sonnet"
            echo "  フルパワーモード（--kessen）: 全エグゼキューター=Opus"
            echo ""
            echo "表示モード:"
            ...
            echo ""
            echo "エイリアス:"
            echo "  csst  → cd /mnt/c/tools/multi-agent-shogun && ./launch.sh"
...
# ───────────────────────────────────────────────────────────────────────────────
# 【著作権・ライセンス表示（歴史的注記）】
# 旧バージョンで使用していた忍者ASCIIアート: syntax-samurai/ryu - CC0 1.0 Universal (Public Domain)
...
show_battle_cry() {
    ...
    echo -e "\033[1;31m║\033[0m \033[1;33m╚══════╝╚═╝  ╚═╝ ╚═════╝    ╚═╝   ╚══════╝ ╚═════╝  ╚════╝ ╚═╝╚═╝  ╚═══╝\033[0m \033[1;31m║\033[0m"
    echo -e "\033[1;31m╠══════════════════════════════════════════════════════════════════════════════════╣\033[0m"
    echo -e "\033[1;31m║\033[0m       \033[1;37m起動開始！\033[0m    \033[1;36m🚀\033[0m    \033[1;35mLet's ship it!\033[0m                          \033[1;31m║\033[0m"
    echo -e "\033[1;31m╚══════════════════════════════════════════════════════════════════════════════════╝\033[0m"
    echo ""
    ...
    echo -e "\033[1;34m  ║\033[0m              \033[1;37m【 精 鋭 チ ー ム ・ 起 動 準 備 完 了 】\033[0m                    \033[1;34m║\033[0m"
    echo -e "\033[1;34m  ╚═════════════════════════════════════════════════════════════════════════════╝\033[0m"

    # ═══════════════════════════════════════════════════════════════════════════
    # ドットキャラクター表示（TrueColor ANSI art — pixel-characters.html由来）
    # ═══════════════════════════════════════════════════════════════════════════
    if [ -f "$SCRIPT_DIR/scripts/generate_banner_art.py" ]; then
        "$SCRIPT_DIR/.venv/bin/python3" "$SCRIPT_DIR/scripts/generate_banner_art.py" --preview
    fi
    echo ""
    echo -e "                    \033[1;36m「「「 了解！起動します！ 」」」\033[0m"
    echo ""
    ...
    echo -e "\033[1;33m  ┃\033[0m  \033[1;37m🚀 multi-agent-shogun\033[0m  〜 \033[1;36m精鋭チーム マルチエージェント\033[0m 〜                 \033[1;33m┃\033[0m"
    echo -e "\033[1;33m  ┃\033[0m                                                                           \033[1;33m┃\033[0m"
    echo -e "\033[1;33m  ┃\033[0m  \033[1;33m総統\033[0m(Opus) \033[1;37mハク\033[0m(Opus) \033[1;32mゴーグル\033[0m(Haiku) \033[1;31mリキニキ\033[0m \033[1;34mアオさん\033[0m \033[0;35mブラッキー\033[0m(Sonnet) \033[1;33m┃\033[0m"
    echo -e "\033[1;33m  ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛\033[0m"
    echo ""
}
...
echo -e "  \033[1;33mLet's ship it! セットアップを開始します\033[0m (Setting up the environment)"
echo ""

# ═══════════════════════════════════════════════════════════════════════════════
# STEP 1: 既存セッションクリーンアップ
# ═══════════════════════════════════════════════════════════════════════════════
log_info "🧹 既存セッションをシャットダウン中..."
tmux kill-session -t multiagent 2>/dev/null && log_info "  └─ multiagentセッション、シャットダウン完了" || log_info "  └─ multiagentセッションは存在せず"
tmux kill-session -t shogun 2>/dev/null && log_info "  └─ shogunセッション、シャットダウン完了" || log_info "  └─ shogunセッションは存在せず"
...
if [ "$CLEAN_MODE" = true ]; then
    log_info "📜 前回のキューをリセット中..."

    # エグゼキューター用タスクファイルリセット
...
EOF
    ...
    # agent inbox リセット
    for agent in shogun karo $_ASHIGARU_IDS_STR; do
        echo "messages:" > "./queue/inbox/${agent}.yaml"
    done

    log_success "✅ リセット完了"
else
    log_info "📜 前回の状態を維持して起動..."
...
## 🚨 要対応 - ユーザーのご判断をお待ちしています (Action Required - Awaiting User's Decision)
なし (None)
...
# ═══════════════════════════════════════════════════════════════════════════════
log_war "👑 オーケストレーターのメインセッションを構築中..."
...
log_success "  └─ オーケストレーターのメインセッション、構築完了"
echo ""
...
# ═══════════════════════════════════════════════════════════════════════════════
log_war "⚔️ コーディネーター・エグゼキューターの構成を構築中（5名配備）..."
...
tmux split-window -v -t multiagent:agents.$((PANE_BASE+2))
tmux split-window -v -t multiagent:agents.$((PANE_BASE+1))

# レイアウトを均等に
tmux select-layout -t multiagent:agents tiled

# ペインラベル・エージェントID・色設定（精鋭チーム）
PANE_LABELS=("ハク" "ゴーグル" "リキニキ" "アオさん" "ブラッキー")
AGENT_IDS=("karo" "ashigaru1" "ashigaru2" "ashigaru3" "ashigaru4")
PANE_COLORS=("white" "green" "red" "blue" "magenta")

# モデル名設定（pane-border-format で常時表示するため）
# 通常モード: コーディネーター=Opus, エグゼキューター1=Haiku, エグゼキューター2-4=Sonnet
# フルパワーモード: 全員Opus
ASHIGARU1_MODEL="claude-haiku-4-5-20251001"
ASHIGARU2_MODEL="claude-sonnet-4-6"
ASHIGARU3_MODEL="claude-sonnet-4-6"
ASHIGARU4_MODEL="claude-sonnet-4-6"

if [ "$KESSEN_MODE" = true ]; then
    ASHIGARU1_MODEL="claude-opus-4-6"
    ASHIGARU2_MODEL="claude-opus-4-6"
    ASHIGARU3_MODEL="claude-opus-4-6"
    ASHIGARU4_MODEL="claude-opus-4-6"
fi

MODEL_NAMES=("opus-4.6" "haiku-4.5" "sonnet-4.6" "sonnet-4.6" "sonnet-4.6")
if [ "$KESSEN_MODE" = true ]; then
    MODEL_NAMES=("opus-4.6" "opus-4.6" "opus-4.6" "opus-4.6" "opus-4.6")
fi
...
for i in "${!AGENT_IDS[@]}"; do
    p=$((PANE_BASE + i))
    tmux select-pane -t "multiagent:agents.${p}" -T "${PANE_LABELS[$i]}"
    ...
    tmux send-keys -t "multiagent:agents.${p}" "cd \"$(pwd)\" && export PS1='${PROMPT_STR}' && clear && \"${SCRIPT_DIR}/.venv/bin/python3\" \"${SCRIPT_DIR}/scripts/generate_banner_art.py\" --agent ${AGENT_IDS[$i]}" Enter
done

# pane-border-format でモデル名を常時表示
tmux set-option -t multiagent -w pane-border-status top
tmux set-option -t multiagent -w pane-border-format '#{?pane_active,#[reverse],}#[bold]#{pane_title}#[default] (#{@model_name}) #{@current_task}'

log_success "  └─ 精鋭チームの構成、構築完了"
echo ""
...
if [ "$SETUP_ONLY" = false ]; then
    ...
    log_war "👑 精鋭チームに Claude Code を起動中..."

    # オーケストレーター: CLI Adapter経由でコマンド構築
    _shogun_cli_type="claude"
    _shogun_cmd="claude --model claude-opus-4-6 --dangerously-skip-permissions"
    if [ "$CLI_ADAPTER_LOADED" = true ]; then
    ...
    if [ "$SHOGUN_NO_THINKING" = true ] && [ "$_shogun_cli_type" = "claude" ]; then
        ...
        log_info "  └─ ★総統（${_shogun_cli_type} / thinking無効）、起動完了"
    else
        tmux send-keys -t shogun:main "$_shogun_cmd"
        tmux send-keys -t shogun:main Enter
        log_info "  └─ ★総統（${_shogun_cli_type}）、起動完了"
    fi

    # 少し待機（安定のため）
    sleep 1

    # コーディネーター（pane 0）: CLI Adapter経由でコマンド構築（デフォルト: Sonnet）
    p=$((PANE_BASE + 0))
    _karo_cli_type="claude"
    _karo_cmd="claude --model claude-opus-4-6 --dangerously-skip-permissions"
    if [ "$CLI_ADAPTER_LOADED" = true ]; then
    ...
    log_info "  └─ ハク（${_karo_cli_type}）、起動完了"

    # --- エグゼキューター起動（モデル個別指定）---
    _ASHI_MODELS=("$ASHIGARU1_MODEL" "$ASHIGARU2_MODEL" "$ASHIGARU3_MODEL" "$ASHIGARU4_MODEL")

    if [ "$KESSEN_MODE" = true ]; then
        # フルパワーモード: 全エグゼキューターOpus
        for i in $(seq 1 "$_ASHIGARU_COUNT"); do
            p=$((PANE_BASE + i))
            _ashi_cli_type="claude"
            _ashi_cmd="claude --model claude-opus-4-6 --dangerously-skip-permissions"
            if [ "$CLI_ADAPTER_LOADED" = true ]; then
        ...
        log_info "  └─ ゴーグル・リキニキ・アオさん・ブラッキー（フルパワーモード・全員Opus）、起動完了"
    else
        # 通常モード: エグゼキューター1=Haiku, エグゼキューター2-4=Sonnet
        for i in $(seq 1 "$_ASHIGARU_COUNT"); do
            p=$((PANE_BASE + i))
            _model="${_ASHI_MODELS[$((i-1))]}"
            _ashi_cli_type="claude"
            _ashi_cmd="claude --model $_model --dangerously-skip-permissions"
            if [ "$CLI_ADAPTER_LOADED" = true ]; then
                _ashi_cli_type=$(get_cli_type "ashigaru${i}")
                _ashi_cmd=$(build_cli_command "ashigaru${i}")
            fi
        ...
        log_info "  └─ ゴーグル(Haiku)+リキニキ・アオさん・ブラッキー(Sonnet)、起動完了"
    fi

    if [ "$KESSEN_MODE" = true ]; then
        log_success "✅ フルパワーモードで起動！精鋭チーム、全員Opus！"
    else
        log_success "✅ 通常モードで起動（総統+ハク=Opus, ゴーグル=Haiku, リキニキ+アオさん+ブラッキー=Sonnet）"
    fi
    echo ""
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # 起動完了バナー（旧バージョンでは忍者ASCIIアート syntax-samurai/ryu CC0 1.0 を使用）
    # ═══════════════════════════════════════════════════════════════════════════
    echo -e "\033[1;35m  ┌────────────────────────────────────────────────┐\033[0m"
    echo -e "\033[1;35m  │\033[0m  \033[1;37m🚀 All agents launched. Let's ship it! 🚀\033[0m      \033[1;35m│\033[0m"
    echo -e "\033[1;35m  └────────────────────────────────────────────────┘\033[0m"
    ...
    # オーケストレーターのwatcher（ntfy受信の自動起床に必要）
    ...
    log_success "  └─ $((_ASHIGARU_COUNT + 2))エージェント分のinbox_watcher起動完了（オーケストレーター+コーディネーター+エグゼキューター${_ASHIGARU_COUNT}）"
...
echo ""
echo "  ┌──────────────────────────────────────────────────────────┐"
echo "  │  📋 チーム構成図 (Formation)                              │"
echo "  └──────────────────────────────────────────────────────────┘"
echo ""
echo "     【shogunセッション】★ オーケストレーターのメインセッション"
echo "     ┌─────────────────────────────────┐"
echo "     │  Pane 0: ★ 総統 (Opus 4.6)     │  ← 指揮官・プロジェクト統括"
echo "     └─────────────────────────────────┘"
echo ""
echo "     【multiagentセッション】精鋭チームの構成（5ペイン）"
echo "     ┌────────────────┬────────────────┐"
echo "     │  ハク (コーディネーター)│ リキニキ (エグゼキューター2) │"
echo "     │  Opus 4.6      │ Sonnet 4.6     │"
echo "     │  📋 管理役     │ 💪 主力実装     │"
echo "     ├────────────────┼────────────────┤"
echo "     │ ゴーグル (エグゼキューター1) │ アオさん (エグゼキューター3) │"
echo "     │  Haiku 4.5     │ Sonnet 4.6     │"
echo "     │  👓 偵察役     │ 🔍 分析役      │"
echo "     ├────────────────┼────────────────┤"
echo "     │ ブラッキー (エグゼキューター4) │               │"
echo "     │  Sonnet 4.6    │               │"
echo "     │  🛡️ 門番役     │               │"
echo "     └────────────────┴────────────────┘"
echo ""
...
echo ""
echo "  ════════════════════════════════════════════════════════════"
echo "   Let's ship it! Ship great code!"
echo "  ════════════════════════════════════════════════════════════"
echo ""
